...
{if $current_user->isLoggedIn()}
    ...
    {/if}
{else}
    <strong class="just-rating{if $comment->getRating() < 0} bad{elseif $comment->getRating() > 0} good{/if}">{if $comment->getRating() > 0}+{elseif $comment->getRating() < 0}&minus;{/if}{$comment->getRating()|@abs}</strong>
{/if}
{/strip}